...
This vignette details how to use EndpointR to embed and classify text using Hugging Face's inference services.
...
[Hugging Face](https://huggingface.co/) is the primary hub for open-source AI models. You can find, use, and share thousands of pre-trained models for tasks like text embedding, classification, generation, and more.

## Inference Options - Hugging Face
...
EndpointR's Hugging Face functions work with the Inference API and Dedicated Inference Endpoints - you just need to change the URL in the `endpoint_url =` argument!

# Getting Started

We'll use the Hugging Face Inference API to classify and embed some data.
...
```{r}
embedding_url <-"https://router.huggingface.co/hf-inference/models/sentence-transformers/all-mpnet-base-v2/pipeline/feature-extraction"
```
...
embeddings_df <- hf_embed_df(
  ...
  key_name = "HF_TEST_API_KEY",
  endpoint_url = embedding_url,
  progress = FALSE,     # Set TRUE for progress bar with large datasets
  max_retries = 5
)
```
...
We need to switch our URL to a model which classifies text. For demonstrative purposes we'll use 'distilbert-base-uncase-fintuned-sst-2-english'.

> **NOTE**: Not all models are deployed on the Inference API; for models which are not deployed by Hugging Face, we need to deploy Dedicated Inference Endpoints.
...
sentiment <- hf_classify_text(
  "this is a *terrible* post to classify",
  endpoint_url = sentiment_url,
  key_name = "HF_TEST_API_KEY",
  tidy = TRUE
)
```
...
```{r}
classification_df <- hf_classify_df(
  df = my_data,
  text_var = text,      # Column containing text
  id_var = id,          # Column with unique IDs
  key_name = "HF_TEST_API_KEY",
  endpoint_url = sentiment_url,
  progress = TRUE,     # Set TRUE for progress bar with large datasets
  max_retries = 5
)
```

```{r}
use_data(df_sentiment_classification_example)
df_sentiment_classification_example
```

# Going Deeper

In this section we'll look into Dedicated Inference Endpoints, and how to squeeze out performance from EndpointR.

# Going Deeper - Dedicated Inference Endpoints

The Hugging Face Inference API is a great place to begin experimenting with Hugging Face's inference services. However, it is best used as a casual service, rather than for production-grade applications. When we want cast-iron guarantees about response time, availability, and model selection, we need to move to production-ready inference services, such as Dedicated Inference Endpoints.
...
From a user's perspective, Dedicated Inference Endpoints are very similar to the Inference API. In fact, we just need to change the value of `endpoint_url =`.

> For this example, I have previously set up a package-level encryption key (`ENDPOINTR_KEY`) for protecting sensitive information, like private endpoint URLs, and API keys. To follow this vignette you will need to supply your own Dedicated Infernece API URL.

<details>

<summary>Creating and using a package-level encryption key</summary>

Why? It's good practice to avoid exposing the locations of private API endpoints, as well as API keys.

How?

1.  Create a key with `httr2::secret_make_key()`
2.  Add it to your .Renviron file
3.  Encrypt the information with `httr2::secret_encrypt(info, ENDPOINTR_KEY)`
4.  Paste the encrypted information wherever you need it
5.  Decrypt with `httr2::secret_decrypt(encrypted_info, ENDPOINTRL_KEY)`

Note, the name `ENDPOINTR_KEY` is just what I chose, you can choose whatever you like. If you try to use `set_api_key()` you'll need to make sure it endsw with `API_KEY`. You can encrypt and decrypt *any* information you like using your key and the {httr2} functions above.
...
EndpointR functions that need API keys will have a `key_name` argument, you put the same name as you entered into `set_api_key` and EndpointR will retrieve it, without printing the contents to the console. Read the [EndpointR API Keys](articles/api_keys.html) vignette for more information
