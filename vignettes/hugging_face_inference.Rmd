...
In a separate session we ran some benchmarking to understand the relationship between batch_size and concurrent_requests.
...
Although we suspect there is *some* interaction effect, the relationship is mostly linear within our constraints, i.e. increasing batch size increases throughput, as does increasing concurrent requests. Concurrent requests has a more pronounced effect than batch size.
