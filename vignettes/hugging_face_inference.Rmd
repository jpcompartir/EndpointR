...
Sending concurrent requests is one way to be more efficient - mainly through increasing throughput. Another way to be more efficient would be to send more data inside each request, thus reducing total number of requests, and time spent sending requests to the endpoint.

To be concrete, this means sending multiple rows of data inside each request. If we have 1,000 rows of data, and we create batches of size 50, we'll have 1,000/50, or 20 total requests to send, rather than 1,000 requests.

Endpoints powered by GPUs (or TPUs) make use of parallel computing to speed up computation of requests. Powerful GPUs can deal with 100s - 1000s of requests concurrently

```{r}
# process in batches with multiple concurrent requests
result_df <- hf_embed_df(
  df = texts_df,
  text_var = text,
  id_var = id, 
  ...
  concurrent_requests = 5, # maximum 5 concurrent requests
