...
This vignette is for users needing to embed or classify data using pre-configured, dedicated Hugging Face Endpoints, assuming you have received your API key and the endpoint URL/details.

Experienced users will most likely prefer to head directly to `httr2`.
...
You can also pass in a value for 'path', which will save the response to a file, we'll look more at this later.
...
And so on, for as many texts as we have. But this would very quickly get old. Instead we want to process multiple texts in one function call.

### Embedding a Batch of Texts

TODO: just cover a single batch

Rather than send 10 requests and get 10 responses, we could batch our texts together into 1 request, send it to the endpoint, and let it handle the batch together. If done correctly, this will tend to lead to large efficiency gains.
...
the `hf_embed_batch()` function handles a list of texts, and turns them into batches, sized according to what we enter into `batch_size =`. The default value is 8, we'll change it to 10 and send all of our texts in one batch.
...
We get a data frame where the first column is our list texts, and the rest of the columns are the embedding dimensions.

Batching is generally a good idea, and we'll see later that holdimg everything else equal, it does lead to a speed up over non-batched. However, if you try to set the batch_size too high, you are more likely to hit rate limits, or have your request rejected due to the payload being too large.
...
TODO: let's only cover a data frame, no batched df/concurrency, leave that for scaling request
...
And then we have some optional arguments - these are optional because there is a default value for when we don't set them. The key optional arguments for `hf_embed_df()` are:

-   `batch_size` lets EndpointR know whether to send requests individually or in batches.
...
> **NOTE**: We'll look at some additional arguments, such as batch_size and concurrent_requests later in [scaling performance](scaling-requests---improving-performance)
...
## Scaling Requests - Improving Performance
...
TODO: fix follwing all the re-factoring Prepare requests without executing them, useful for inspection or custom processing:
