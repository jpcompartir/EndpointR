...
This vignette shows you how to use EndpointR to embed and classify text using Hugging Face's inference services.
...
| **Wait Times** | Variable, unknowable in advance | Predictable, minimal queuing |
...
| **Model Coverage** | Selection of the commonly-used models | Virtually all mdoels on the Hub are available |

EndpointR works with both - you just need to change the URL!
...
Follow Hugging Face's [docs](https://huggingface.co/docs/hub/security-tokens) to generate a Hugging Face token.
...
> **NOTE**: Not all models are deployed on the Inference API, for models which are not we need to deploy Dedicated Inference Endpoints
...
First read the [EndpointR API Keys](articles/api_keys.html) vignette.
