...
Anthropic offers a [Message Batches API](https://docs.anthropic.com/en/docs/build-with-claude/batch-processing) with 50% cost savings, up to 100,000 requests per batch, and a 24-hour completion window. Unlike OpenAI's file-upload workflow, Anthropic's API accepts requests directly — no file upload step is needed.

The workflow is three compositional steps: **create**, **check status**, and **retrieve results**.

## Batch Messages

```{r ant-batch-messages, eval = FALSE}
# 1. Create a batch
...
Structured outputs work with the Anthropic Batches API too. Pass a `json_schema` object to the `schema` parameter:

```{r ant-batch-schema, eval = FALSE}
# 1. Define a schema
sentiment_schema <- create_json_schema(
  name = "sentiment_analysis",
  schema_object(
...
<details>
  <summary>In Progress Response</summary>
...
<details> 
  <summary> Complete Repsonse </summary>
  ```
  {
    "id": "xxx",
    "type": "message_batch",
...
```{r ant-batch-management, eval = FALSE}
# List recent batches
...
```{r ant-batch-large-scale, eval = FALSE}
...
> **TIP**: Prompt caching kicks in automatically when you provide a `system_prompt`, so the repeated system prompt across all 500k requests is cached — saving both latency and cost on top of the 50% batch discount.
...
> For more detail, see [OpenAI's Batch guide](https://platform.openai.com/docs/guides/batch) and [Anthropic's Batch Processing guide](https://docs.anthropic.com/en/docs/build-with-claude/batch-processing).
