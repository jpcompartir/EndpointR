...
```{r}
mapped_responses <- tibble(
  text = classical_texts
) |> 
  mutate(request = map(text, ~ oai_build_completions_request(input = .x, system_prompt = updated_sentiment_prompt)
  )) |> 
  mutate(response = map(request, perform_request_or_return_error, .progress = TRUE)) 
```

```{r}
pluck_text <- function(x) pluck(x, "choices", 1, "message", "content")
